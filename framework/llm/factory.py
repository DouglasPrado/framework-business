...
def _build_callbacks(cfg: Mapping[str, Any]) -> List[Any]:
    ...
    # Adicionar callback de monitoramento se habilitado
    callbacks.extend(_monitoring_callbacks(cfg))
...
def _monitoring_callbacks(cfg: Mapping[str, Any]) -> List[Any]:
    """Cria callback de monitoramento."""
    try:
        from framework.observability.callbacks import create_monitoring_callback
        ...
        # Extrair contexto do agente se disponível
        agent_context = cfg.get("agent_context", {})

        return [create_monitoring_callback(agent_context)]
    except ImportError:
        # Se módulo de monitoramento não estiver disponível, retornar vazio
        return []
