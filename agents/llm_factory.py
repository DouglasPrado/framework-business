...
def build_llm(config: Optional[Mapping[str, Any]] = None) -> Any:
    """Constroi um LLM configurado para uso pelos agentes.

    Parameters
    ----------
    config:
        Dicionário com parâmetros adicionais. Chaves reconhecidas:
...
def _resolve_chat_openai():
    ...
    raise RuntimeError(
        "Dependência langchain_openai não encontrada. Instale langchain-openai para usar ChatOpenAI."
    )
...
def _build_callbacks(cfg: Mapping[str, Any]) -> List[Any]:
    ...
    if isinstance(user_callbacks, Iterable) and not isinstance(user_callbacks, (str, bytes)):
...
def _langchain_callbacks(observability: Mapping[str, Any]) -> List[Any]:
    ...
    if LangChainTracer is None:
        raise RuntimeError(
            "LangChainTracer solicitado mas langchain não está instalado com suporte a tracing."
        )
    if tracer_cfg is True:
        ...
        raise TypeError("Configuração de langchain_tracer deve ser bool, str ou mapeamento.")
